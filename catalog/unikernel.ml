...
module Catalog_Store = struct
  ...
  let con_lst = [
    "review", "https://localhost:4433/";
  ]

  let gen_cmap ctx l =
    let map = ConvenerMap.empty in
    Lwt_list.fold_left_s (fun acc (category, uri) ->
      let root = Uri.of_string uri in
      Convener.create ~category ~ctx ~root
      >>= fun con ->
      ConvenerMap.add category con acc
      |> return) map l
  ...
  let init ~time ~ctx () =
    S.make ~owner:"ucn.catalog" ~time ~check () >>= fun store ->
    gen_cmap ctx con_lst >>= fun cmap ->
    let muse_root = Uri.of_string "https://localhost:12345" in
    Muse.init ~ctx ~root:muse_root >>= fun muse ->
    return (store, muse, cmap)
end
...
module Main
  ...
  let start http resolver conduit keys _clock =
    ...
    Catalog_Store.init ~time ~ctx () >>= fun s ->
    ...
    Lwt.join [
      ...
      http (`TCP 9081) @@ D.serve D.redirect
    ]
end
